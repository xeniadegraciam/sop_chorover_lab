...
This technique applies a heat source that bounce the molecules inside your sample (the red laser observed is not the source, it is just for guidance).
Part of the heat is going through the sample and part of it is surrounding it.
Then the waves converge together and they are cancelling and constructing each other.
That’s when the Fourier transform computation happens in the computer, bringing up the plot of **Absorbance VS. Wavelength**.
Some molecules don’t respond to heat, nitrogen in the air doesn’t, so that’s why there is a nitrogen vacuum inside the machine, because it wont affect your sample measurement.
...
*NOTE*: To erase the background from the plot so it doesn’t bother you, you can select it until it is red color, and then go to:
Edit -> Cut.
...
- This software is used to analize the sample files.
...
- Metal Free Tubes maximum is 1200 RCF.
- Ultra 30 mL darkish tubes, have a maximum of 1500 RCF.
...
Airlock
On (to extract the oxygen). Cycle the Airlock.
