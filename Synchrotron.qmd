...
::: callout-important
Refill the Germanium detector with LN2 once every 24 hours.
:::
...
- Move -> Apply
- MONO position should be now 11919 on the uXaS GUI.
...
***Here we tell the program where to find the position of the center of the samples we need to run. We scan all the holder and select the center of each window.***
...
- Using the **plot screen, find the center of each window**, by finding the highest position of each peak. **Take notes** of each position for future reference.
...
- In here we use the detector, so we need to **uncover** it, and add the Iron foil filter.
- Go to Scan Motor -> Motors -> Detector. 
- Check your detector **current** position.
- Check your Scan Type is **ABSOLUTE**.
- Move the detector position to get it as close as possible to the sample. When you see the Iron foil is moving a little, it is close enough.
...
- Go to **Scan Motor -> Motors -> VERT_ADC -> Scan Type:Absolute -> Move to:** *Move to the position of the first sample you are analizing.*
- Wait for the message saying: ***"Moving completed"***.
- SVGAP is ALWAYS 2.
- Go to **SHGAP** and insert a small value if the concentration of your sample is high, for example: 3.
- Wait for the message saying: ***"Moving completed"***.
- In uMap Gui, click **START**.
- **Check the 100 channels** in uMap Gui, none should be red.
...
- **Scan Motor -> Motors -> VERT_ADC -> Scan Type:Absolute -> Move to:**
- Wait for the message saying: ***"Moving completed"***.
- In uMap Gui, click **START**.
- **Check the 100 channels** in uMap Gui, none should be red.
- Check that your total ICR counts have NOT overpass the 100K counts.
...
- Got to the **Ionic Chamber Control** (Channel 1 to 5), and play with the **"Gain"** until all the channels are below 5, and as close to zero as possible, but never zero.
- Offset -> Detector -> Settings -> Offset -> Counts: 10. **Do not press DONE yet**
- Wait for 10 seconds until the Lead shutters are down, interrupting the beamline.
- Click "DONE".
...
- Setup Queue -> Insert names - Save Queue
- Accept
...
- Pick up some Liquid Nitrogen from the LN2 station. Use mask and gloves.
...
- Take out the cryostat. Unscrew it and place those scres in a known place. DO NOT loose them.
- Take the new cryostat and oout it on position. Screw it on 2 corners.
- Connect the vacuum tube again.
- Open the valv of the pump and Start pump. A vacuum of 10-7 or 10-6 is good for LN2.
...
- Refill the Germanium Detector with LN2 once every 24 hours.
- Take out the cover from the detector. Be careful t not bring the metallic cover, jut the one you used to protect it.
- Close the hutch.
